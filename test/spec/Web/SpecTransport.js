...
describe('Transport', function () {
    ...
    describe('.loadConfig', function () {
        ...
        it('sets default settings for many parameters', function () {
            ...
            expect(this.ua.transport.configuration.wsServers).toEqual([{
                scheme: 'WSS',
        ...
        it('throws a configuration error when a mandatory parameter is missing', function () {
            spyOn(this.ua.transport, 'getConfigurationCheck').and.returnValue({
                mandatory: {
                    fake: function (value) {
                        return;
                    }
            ...
            }.bind(this)).toThrowError('Missing parameter: fake');
        });
        ...
        it('throws a configuration error if a mandatory parameter\'s passed-in value is invalid', function () {
            ...
            }.bind(this)).toThrowError('Invalid value "fake" for parameter \'fake\'');
        });
        ...
        xit('makes sure the connection recovery max interval is greater than the min interval', function () {
            expect(function () {
                this.ua.transport.loadConfig({connectionRecoveryMaxInterval: 1, connectionRecoveryMinInterval: 2});
            }.bind(this)).toThrowError('Invalid value 1 for parameter "connectionRecoveryMaxInterval"');
        });
    ...
    describe('.configurationCheck', function () {
        ...
        describe('.wsServers', function () {
            ...
            it('returns correctly if none of the above is wrong', function () {
                expect(configCheck.optional.wsServers([{wsUri: 'wss://edge.sip.onsip.com'}])).toEqual([{
                    wsUri: 'wss://edge.sip.onsip.com',
                    sipUri: '<sip:edge.sip.onsip.com;transport=ws;lr>',
                    weight: 0,
                    scheme: 'WSS',
                ...
                expect(configCheck.optional.wsServers("wss://edge.sip.onsip.com")).toEqual([{
                    wsUri: 'wss://edge.sip.onsip.com',
                    sipUri: '<sip:edge.sip.onsip.com;transport=ws;lr>',
                    weight: 0,
                    scheme: 'WSS',
        ...
        describe('.disconnectTimeout', function () {
            it('fails for anything but numbers', function () {
                expect(configCheck.optional.disconnectTimeout(true)).toBeUndefined();
                expect(configCheck.optional.disconnectTimeout('string')).toBeUndefined();
                expect(configCheck.optional.disconnectTimeout(['arrays'])).toBeUndefined();
                expect(configCheck.optional.disconnectTimeout({even: 'objects'})).toBeUndefined();
            });

            it('fails for negative numbers and 0', function () {
                expect(configCheck.optional.disconnectTimeout(0)).toBeUndefined();
                expect(configCheck.optional.disconnectTimeout(-7)).toBeUndefined();
            });

            it('passes for positive numbers', function () {
                expect(configCheck.optional.disconnectTimeout(7)).toBe(7);
            });
        });
